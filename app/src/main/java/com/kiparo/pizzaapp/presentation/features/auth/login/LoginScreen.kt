...
@Composable
fun Login(
    modifier: Modifier, uiState: LoginUiState,
    onMailChange: (String) -> Unit,
    onPasswordChange: (String) -> Unit,
    onFocusChange: (String) -> Unit,
    onSubmit: () -> Unit
...
//                    }
        ...
        Row(modifier = Modifier.padding(start = padding_64, end = padding_64, top = padding_8)) {
            Text(
            ...
            Spacer(modifier = Modifier.width(space_4))
            ClickableText(
                text = AnnotatedString(stringResource(id = R.string.reset)),
                style = MaterialTheme.typography.bodyLarge.copy(
                    fontWeight = FontWeight.W600,
                    color = MaterialTheme.colorScheme.background
                ),
...
fun loginScreenPreview() {
    KiparoPizzaAppTheme {
    LoginScreen(uiState = LoginUiState(mail = "mail", password = "password"), onMailChange = {} , onPasswordChange ={} , onFocusChange = {} ) {
